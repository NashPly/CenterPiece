    ...
    public JSONObject getCardDestinationFromItemCodeResult(){
        ...
        switch (this.itemGroup) {
            ...
            case "3500" -> {
                ...
                if(this.agilityItemSearchResult.getString("ExtendedDescription").matches("[F,f][A,a][B,b] - .*\\d{3,4}[K,k]?-\\d{2}.*") || this.agilityItemSearchResult.getString("ExtendedDescription").contains("[S,s][L,l][A,a][B,b] - .*\\d{3,4}-\\d{2}.*")){
